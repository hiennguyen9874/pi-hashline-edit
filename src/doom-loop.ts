import { SUGGESTIONS, GENERIC_SUGGESTION } from "./doom-loop-suggestions";
...
function renderCompactStep(toolName: string, input: Record<string, unknown>): string {
  ...
  for (const key of salient) {
    ...
    if (candidate.length > COMPACT_LINE_BUDGET) {
      ...
      line = `${line} ${key}=${truncate(rendered, Math.max(3, remaining))}`;
      return line;
    }
    line = candidate;
  }
...
export function resetDoomLoopStateForTests(): void {
  globalDoomLoopState.recentCalls = [];
  globalDoomLoopState.pendingWarnings.clear();
}
